...
HELPER="$CMREMIX_ROOT"/vendor/cmremix/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
setup_vendor "$DEVICE/$PLATFORM/$SUBSYSTEM" "$VENDOR" "$CMREMIX_ROOT" true true $SUBSYSTEM
...
cat << EOF > "$CMREMIX_ROOT/vendor/$VENDOR/$DEVICE/Android.mk"
